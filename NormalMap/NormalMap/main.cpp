...
bool draw_Objects()
{
	...
	globalLightingEffect->SetValue("f_SpecularPower", &torusSpecPow, sizeof(float));

	globalLightingEffect->SetTechnique("TexturedLit");
	UINT numPasses = 0;
...
bool initResources()
{
	...
	if (!meshTorusKnot.Load()){MessageBox(NULL, "Unable to load TorusKnot.x", "Error", MB_OK); return false;}
		if (!meshTorusKnot.ComputeTangentSpace()){MessageBox(NULL, "Can't compute tangent space for 'TorusKnot.x'", "Error", MB_OK); return false;}
